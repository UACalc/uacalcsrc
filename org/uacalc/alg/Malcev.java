...
public class Malcev {
 ...
 // ** Need to put in a test if the tables will fit in memory. **
  ...
  /** 
   * Alexandr Kazda's implementation (based on bits of previous UACalc code)
   * of the local to global test for quasi weak near uninimity operation of
   * arity <code>arity</code>. See the paper A. Kazda "Deciding the existence
   * of quasi weak near unanimity terms in finite algebras"
   *
   * 
  ...
  public static boolean fixedKQwnu(SmallAlgebra testalg, int arity, ProgressReport report) {
    ...
    if (report != null) report.addStartLine("Using the local to global method to test for quasi WNU of arity " + arity + ". See the paper A. Kazda \"Deciding the existence of quasi weak near unanimity terms in finite algebras,\" 2020."); 
    BigProductAlgebra alg = new BigProductAlgebra(testalg, arity);
    final int size = testalg.cardinality();
    //IntArray genvector = new IntArray(arity);
    int a;
    int b;
    int[][] generatingMatrix= new int[arity][arity];
    int[] oneColumn=new int[arity];
    boolean found;
    for (a=0;a<size;a++) {
      Arrays.fill(oneColumn,a);
      for (int i=0;i<arity;i++)
        System.arraycopy(oneColumn,0,generatingMatrix[i],0,arity);//Fill the whole generatingMatrix with a's
      for(b=0;b<size;b++) {
        if (a==b)
          break;
        List<IntArray> gens = new ArrayList<IntArray>(arity);
        for (int i=0;i<arity;i++) {
          generatingMatrix[i][i]=b; //The diagonal of generatingMatrix is b
          gens.add(new IntArray(generatingMatrix[i]));
        }
        Closer closer = new Closer(alg, gens);
        closer.setSuppressOutput(true);
        List<IntArray> sub = closer.sgClose();
        ...
        if (!found) {
          if (report != null) {
            report.addLine("No diagonal element in the subuniverse generated by the tuples:");
            for (int k = 0; k < arity; k++) {
              String GenString = Arrays.toString(generatingMatrix[k]);
              report.addLine(GenString);
            }
            report.addEndingLine("There is no " + arity + "-ary quasi WNU.");
          }
          else {
            System.out.println("No diagonal element in the subuniverse generated by the tuples:");
            for (int k = 0; k < arity; k++) {
              String GenString = Arrays.toString(generatingMatrix[k]);
              System.out.println(GenString);
            }
  ...
  if (report != null)
    report.addEndingLine("There is a " + arity + "-ary quasi WNU.");
    return true;
}
...
//System.out.println("current level size = " + currentLevel.size());
  ...
  public static Term differenceTerm(SmallAlgebra alg, ProgressReport report) {
    if (alg.cardinality() == 1)  return Variable.x;
    if (report != null) report.addStartLine(
        "Looking for a difference term: d(x,x,y) = y and d(x,y,y) [theta,theta] x, theta = Cg(x,y).");
    FreeAlgebra f2 = new FreeAlgebra(alg, 2, report);
    // ** Need to put in a test if the tables will fit in memory. **
    f2.makeOperationTables();
    //logger.info("f2 size is " + f2.cardinality());
    ...
    final Partition thetaPrime = f2.con().commutator2(theta, theta);
    ...
    BigProductAlgebra f2pow = new BigProductAlgebra(f2, 2);
    Closer closer = new Closer(f2pow, gens, termMap);
    closer.setProgressReport(report);
    closer.setValues(values);
    closer.setupCongruenceConstraint(thetaPrime, 1, 0); // this is the test that the second coord (index 1) is [theta,theta] related to x
    closer.sgClosePower();
    if (closer.getElementToFind() != null) {
      Term diffTerm = termMap.get(closer.getElementToFind());
      if (report != null) report.addEndingLine("found a difference term: " + diffTerm);
      return diffTerm;
    }
    if (report != null) report.addEndingLine("there is no difference term: ");
  ...
  public static Term minorityTerm(SmallAlgebra alg, ProgressReport report) {
    ...
    FreeAlgebra f2 = new FreeAlgebra(alg, 2, report);
    // ** Need to put in a test if the tables will fit in memory. ** 
    //f2.makeOperationTables(); // eliminating this for now
    //logger.info("f2 size is " + f2.cardinality());
    IntArray g0 = new IntArray(new int[] {0,0,1});
    IntArray g1 = new IntArray(new int[] {0,1,0});
    IntArray g2 = new IntArray(new int[] {1,0,0});
    ...
    termMap.put(g1, Variable.y);
    termMap.put(g2, Variable.z);
    final IntArray yyy = new IntArray(new int[] {1,1,1});
    BigProductAlgebra f2cubed = new BigProductAlgebra(f2, 3);
    // the yx argument means stop if yx is found. 
    // of course we want something different if we want the shortest term.
    List<IntArray> sub = f2cubed.sgClose(gens, termMap, yyy, report);
    //logger.info("sub alg of f2 cubed size is " + sub.size());
    if (sub.contains(yyy)) return (Term)termMap.get(yyy);
    return null;
  }
